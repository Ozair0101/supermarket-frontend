...
const Sales: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm(t('pages.sales.delete_confirm'))) {
      try {
        await deleteSale(id);
        fetchSales();
      } catch (error) {
        console.error('Error deleting sale:', error);
        alert(t('pages.sales.delete_failed'));
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('pages.sales.title')}</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('pages.sales.subtitle')}</p>
        </div>
        <button
                ...
                filteredSales.map((sale) => (
                  <tr key={sale.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      {sale.customer ? sale.customer.name : t('pages.sales.walk_in')}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      {sale.sale_date ? new Date(sale.sale_date).toLocaleDateString() : t('common.n_a')}
                    </td>
