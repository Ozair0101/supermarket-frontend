...
const Customers: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm(t('pages.customers.delete_confirm'))) {
      try {
        await deleteCustomer(id);
        fetchCustomers(); // Refresh the list
      } catch (error) {
        console.error('Error deleting customer:', error);
        alert(t('pages.customers.delete_failed'));
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('pages.customers.title')}</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t('pages.customers.subtitle')}</p>
        </div>
        <button
                ...
                filteredCustomers.map((customer) => (
                  <tr key={customer.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <div className="text-gray-400 dark:text-gray-500">{customer.email}</div>
                    </td>
